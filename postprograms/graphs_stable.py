...
class Data(object):
    ...
    def c_m(self):
        return self.data["Pitch"]/(0.5*self.data.ix[0,"rho"]*(self.data["wind speed"]**2)*self.S*self.c)

    def c_r(self):
        return self.data["Roll"]/(0.5*self.data.ix[0,"rho"]*(self.data["wind speed"]**2)*self.S*self.b)

    def c_n(self):
        return self.data["Yaw"]/(0.5*self.data.ix[0,"rho"]*(self.data["wind speed"]**2)*self.S*self.b)
...
class Offset(Data):
    ...
    def c_m(self):
        return self.data["Pitch"]/(0.5*self.data.ix[0,"rho"]*(10**2)*self.S*self.c)

    def c_r(self):
        return self.data["Roll"]/(0.5*self.data.ix[0,"rho"]*(10**2)*self.S*self.b)

    def c_n(self):
        return self.data["Yaw"]/(0.5*self.data.ix[0,"rho"]*(10**2)*self.S*self.b)
...
class Error(Data):
    def __init__(self, filename, error_filename, linetype, marker, label):
        ...
        self.errcm = np.absolute(self.cm * self.error["Pitch"])
        self.errcr = np.absolute(self.cr * self.error["Roll"])
        self.errcn = np.absolute(self.cn * self.error["Yaw"])
...
def offset():
    # 読み込むファイルを指定
    result = Offset("pre_.csv", "b-", "x", "pre")
    # 図のフォントサイズを指定
    plt.rcParams.update({'font.size': 18})
...
def rawdata():
    # 読み込むファイルを指定
    result = Data("data_.csv", "b-", "x", "data")
    # 図のフォントサイズを指定
    plt.rcParams.update({'font.size': 18})
